...
riemann_solver::w run_sod_shock(std::string sim_name, std::size_t resolution)
{
    ...
    {
        if ((f32)scene_mesh.get_cell_position(i).x / (f32)scene_mesh.cells_size().x < 0.5f)
        {
    ...
    for (std::size_t i = 0; i < scene_mesh.cells_size().x; ++i)
    {
        u64vec2 cell_position = { i, 1 };
        f32 x = (f32)i / (f32)scene_mesh.cells_size().x;
